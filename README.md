...
잘 다루지 않는 신진 작가·중저가 작품대에 집중해 차별화합니다. 여기에 쇼츠 스타일의
실시간 라이브 스트리밍과 3D/AR 작품 감상 기능을 더해 웹앱 하나로 "보고·감상하고·입찰하는"
경험을 제공합니다.
...
합니다. 자세한 배경과 전체 아키텍처는 노션 "아트비드(ArtBid) 프로젝트 길잡이" 문서를
참고하세요.
...
`backend`는 레이어(controller/service/...) 기준이 아니라 **도메인(기능) 기준**으로 먼저 나눴고,
난이도 기준 역할 분담(노션 "역할 분담" 페이지)의 4개 트랙과 1:1로 대응합니다.
...
- PR 제목도 커밋 컨벤션과 동일한 `type: 내용` 형식을 따릅니다.
- PR 본문에는 변경 내용, 관련 이슈/노션 문서 링크, 테스트 방법을 간단히 적습니다.
- 최소 1명 이상의 리뷰 승인 후 병합합니다. 셀프 머지는 지양합니다.
